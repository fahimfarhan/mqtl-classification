...
genome=hg19 # hg38
exp_name=mqtl_classification
# Loop through each window size
for window in "${window_sizes[@]}"; do
    echo "Running scripts with WINDOW size: $window, Gnome: $genome"
    python3 _01_preprocess_cosmopolitan_meqtl.py --WINDOW "$window" --GENOME "$genome" --EXP_NAME "$exp_name"
    python3 _02_generate_positive_and_negative_sequences.py --WINDOW "$window" --GENOME "$genome" --EXP_NAME "$exp_name"
    python3 _04_filter_and_splitting.py --WINDOW "$window" --GENOME "$genome" --EXP_NAME "$exp_name"
done
